...
from typing import Annotated, Any, Optional, Sequence, Type, Union
...
if "postgresql+asyncpg://" not in async_database_uri:
...
class CustomAsyncSession(AsyncSession):
    ...
    async def save(self, obj: ModelType, commit: bool = True, refresh: bool = True) -> ModelType:
        self.add(obj)
        if commit:
            await self.commit()
        if refresh:
    ...
    async def save_all(self, objs: list[ModelType], commit: bool = True, refresh: bool = True) -> list[ModelType]:
        self.add_all(objs)
        if commit:
            await self.commit()
        if refresh:
    ...
    async def find_by_id(self, obj: Type[ModelType], id: Union[UUID, Any], *, populated_fields: Optional[Sequence[Any]] = None) -> Optional[ModelType]:
